    ...
    if (!rdsHost) {
        ...
        if (authn && args) {
            ...
            if (queriedEndpoint) {
                ...
                rdsHost = endpoint ?? `${dbInstanceIdentifier}.${region}.rds.amazonaws.com`;
            }
        } else {
            // Fallback: construct endpoint from instance identifier
            const endpoint = await getRdsEndpoint(
...
export const queryBackendForEndpoint = async (
    ...
    try {
        ...
        if (
            !response ||
            !response.items ||
            !(response.items as unknown[]).length
        ) {
            try {
                if (debug) {
...
export const generateDbAuthTokenWithAutoLogin = async (
    ...
    try {
        ...
        const token = await generateDbAuthToken(details, dbUser, profileName, debug);
        if (debug) {
        ...
        try {
            ...
            print2("Error: Failed to generate database authentication token after login.");
