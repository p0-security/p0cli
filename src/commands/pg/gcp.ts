...
export const extractGcpConnectionDetails = async (
    ...
    if (!instanceConnectionName) {
        ...
        if (debug) {
            print2(
                `Constructed instance connection name: ${instanceConnectionName}`
            );
        }
    } else {
        if (debug) {
...
export const checkGcpApisEnabled = async (
    ...
    try {
        ...
        result.cloudSqlEnabled = enabledServices.some(
            (s) => s.includes("sqladmin.googleapis.com") || s.includes("sql-component.googleapis.com")
        );

        // Check for IAP API
        result.iapEnabled = enabledServices.some(
            (s) => s.includes("iap.googleapis.com")
        );
        ...
        if (!result.cloudSqlEnabled) {
            ...
            print2(`   gcloud services enable sqladmin.googleapis.com --project ${projectId}`);
            print2("");
        }

        if (!result.iapEnabled) {
            print2("");
            print2("WARNING: Identity-Aware Proxy (IAP) API is not enabled for this project.");
            print2("   This API may be required for private CloudSQL instances.");
            print2("");
            print2("   To enable it, run:");
            print2(`   gcloud services enable iap.googleapis.com --project ${projectId}`);
            print2("");
        }
    } catch (error) {
        if (debug) {
 ...
 */
export const ensureCloudSqlProxy = async (
    debug?: boolean
): Promise<string> => {
    try {
...
export const connectToCloudSQLViaProxy = async (
    ...
    try {
        ...
        print2(
            "Setting up application-default credentials for Cloud SQL Proxy..."
        );
        try {
...
export const printGcpConnectionDetails = async (
    ...
    try {
        ...
        print2(
            "Setting up Google Cloud credentials for Cloud SQL Proxy..."
        );
        const { command: loginCommand, args: loginArgs } = gcloudCommandArgs([
            "auth",
            "application-default",
...
${connectionUrl}`;
    ...
    try {
        ...
        print2(`  SSL Mode: ${sslMode}${ssl ? "" : " (Cloud SQL Proxy provides encryption)"}`);
        print2("");
        ...
        print2(
            "The proxy will stop when you close this terminal or press Ctrl+C."
        );
        print2("");
